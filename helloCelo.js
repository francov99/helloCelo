...
async function send(){
    ...
    let andAddress = '0xD86518b29BB52a5DAC5991eACf09481CE4B0710d';
    ...
    let celotx = await goldtoken.transfer(andAddress, amount).send({from: account.address});
    let cUSDtx = await stabletoken.transfer(andAddress, amount).send({from: account.address, feeCurrency: stabletoken.address});
